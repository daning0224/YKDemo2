package com.bawei.ykdemo2.util;
...
public class OkHttpFramework {
     ...
     */
    //1.声明对象（私有化，静态，加锁）
    private static volatile OkHttpFramework okHttpFramework;

    //2.私有化构造方法
    private OkHttpFramework() {
        handler = new Handler(Looper.getMainLooper());
        gson = new Gson();
    }

    //3.提供公共静态的方法
    public static OkHttpFramework getOkHttpFramework() {
        if (null == okHttpFramework) {
            synchronized (OkHttpFramework.class) {
                if (null == okHttpFramework) {
                    okHttpFramework = new OkHttpFramework();
                }
            }
        }
        return okHttpFramework;
    }
